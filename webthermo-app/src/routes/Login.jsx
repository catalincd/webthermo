...
const Login = (props) =>{
  ...
  const SendRequest = (username, pass) =>{
        fetch(`http://localhost:8008/login?username=${username}&password=${pass}`, {method: "POST"})
        ...
        .then(data => {
          ...
          if(data.status){
            ...
            setErrorMessage("Wrong user or pass")
          }
        });
  }
  ...
  return (
    <div className="formContainer">
        <Form onSubmit={OnSubmitCallback} buttonText="Login" errorText={{errorMessage, setErrorMessage}}/>
    </div>
  );
}
